...
export const SavedDashboardsManager: React.FC<SavedDashboardsProps> = ({ onLoadDashboard, onClose }) => {
  const { savedDashboards, deleteDashboard } = useAppStore();
      ...
      <div className="max-h-80 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
        {savedDashboards.map((dash, index) => (
          <div 
            ...
            className="flex items-center justify-between gap-3 p-3 rounded-xl border border-zinc-100 bg-white hover:border-indigo-400/50 dark:bg-zinc-950 dark:border-zinc-900 dark:hover:border-indigo-500/50 transition-all group"
          >
            <button
              onClick={() => {
            ...
            >
              <h5 className="text-xs font-semibold text-zinc-800 dark:text-zinc-200 truncate group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
